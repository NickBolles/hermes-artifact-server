...
const createArtifactSchema = z.object({slug:z.string().min(1).max(100).optional(),title:z.string().min(1).max(200).optional(),expiresAt:z.string().datetime().optional(),allowDirectoryListing:z.boolean().optional(),visibility:z.enum(['bearer','authenticated','restricted']).default('bearer'),allowedUsers:z.array(z.string().min(2).max(64)).max(100).default([])}).strict();
const artifactAccessSchema=z.object({visibility:z.enum(['bearer','authenticated','restricted']),allowedUsers:z.array(z.string().min(2).max(64)).max(100).default([]),expectedUpdatedAt:z.string().datetime()}).strict();
...
function artifactHeaders(res:Response,sandbox=false):void {
  res.set({
    'Content-Security-Policy': `default-src 'self' data: https:; script-src 'self' 'unsafe-inline' https:; style-src 'self' 'unsafe-inline' https:; img-src 'self' data: https:; connect-src 'self' https:; frame-ancestors 'none'; base-uri 'none'${sandbox?'; sandbox allow-scripts allow-forms allow-modals allow-popups allow-downloads':''}`,
    'Referrer-Policy':'no-referrer',
...
function resolveAllowedUserIds(state:StateStore,usernames:string[]):string[] {
  const ids=[...new Set(usernames.map((username)=>username.trim().toLowerCase()))].map((username)=>state.getUserByUsername(username)?.id);
  if(ids.some((id)=>!id))throw new Error('Allowed user not found');
...
export function createApp(options:{state?:StateStore;auth?:AuthService;startWorkers?:boolean}={}) {
  ...
  app.use((req,res,next)=>{
    ...
    const artifactSurface=req.path.startsWith('/a/')||req.path.startsWith('/v/')||req.path.startsWith('/viewer/handoff/')||req.path==='/api'||req.path.startsWith('/api/')||req.path==='/admin/files'||req.path.startsWith('/admin/files/');
  ...
  app.get('/auth/artifacts/authorize/:handoffId',async(req,res,next)=>{try{
    appHeaders(res);
    const handoffId=routeParam(req.params.handoffId);
    const handoff=state.getViewerHandoff(handoffId);
    if(!handoff)return res.status(404).send('Artifact sign-in request expired');
    const current=auth.getRequestSession(req);
    if(!current||current.row.kind!=='full'||current.user.requiresPasskeyEnrollment)return res.redirect(`/login?handoff=${encodeURIComponent(handoffId)}`);
    const meta=await readMeta(safeSlug(String(handoff.artifact_slug)));
    if(!meta||!canUserViewArtifact(meta,current.user))return res.status(403).send('You do not have access to this artifact');
    const exchange=auth.authorizeViewerHandoff(handoffId,current.user.id,current.row.id);
    if(!exchange)return res.status(409).send('Artifact sign-in request was already used');
    state.audit(current.user.id,'artifact.viewer_authorized','artifact',meta.slug);
    return res.redirect(`${config.baseUrl}/viewer/handoff/${encodeURIComponent(exchange)}`);
  }catch(error){next(error)}});

  app.get('/viewer/handoff/:exchange',(req,res)=>{
    artifactHeaders(res);
    const finished=auth.finishViewerHandoff(routeParam(req.params.exchange));
    if(!finished)return res.status(401).send('Artifact sign-in request is invalid or expired');
    auth.setViewerSessionCookie(res,finished.sessionToken);
    return res.redirect(finished.returnPath);
  });
  ...
  app.post('/admin/api/viewer-sessions/:id/revoke',adminMutation,(req,res)=>{const current=res.locals.session as ActiveSession;const id=routeParam(req.params.id);state.revokeViewerSession(id);state.audit(current.user.id,'viewer_session.revoked','viewer_session',id);res.json({ok:true})});
  ...
  app.post('/admin/api/artifacts/:slug/access',adminMutation,async(req,res,next)=>{try{
    const current=res.locals.session as ActiveSession; const slug=safeSlug(routeParam(req.params.slug)); const parsed=artifactAccessSchema.parse(req.body);
    const allowedUserIds=resolveAllowedUserIds(state,parsed.allowedUsers);
    if(parsed.visibility==='restricted'&&allowedUserIds.length===0)return res.status(400).json({error:'Restricted artifacts require at least one allowed user'});
    const {meta,token}=await updateArtifactAccess(slug,{visibility:parsed.visibility,allowedUserIds,expectedUpdatedAt:parsed.expectedUpdatedAt});
    state.audit(current.user.id,'artifact.access_changed','artifact',slug,{visibility:meta.visibility,allowedUserIds:meta.allowedUserIds});
    res.json({ok:true,visibility:meta.visibility,allowedUsers:parsed.visibility==='restricted'?parsed.allowedUsers:[],publicUrl:token?artifactUrl(slug,token):undefined,viewerUrl:meta.visibility!=='bearer'?artifactViewerUrl(slug):undefined});
  }catch(error){next(error)}});

  app.use('/api',requireApiToken);
  app.post('/api/artifacts',async(req,res,next)=>{try{
    const parsed=createArtifactSchema.parse(req.body); const allowedUserIds=resolveAllowedUserIds(state,parsed.allowedUsers);
    if(parsed.visibility==='restricted'&&allowedUserIds.length===0)return res.status(400).json({error:'Restricted artifacts require at least one allowed user'});
    const {meta,token,created}=await createArtifact({...parsed,allowedUserIds});
    res.status(created?201:200).json({slug:meta.slug,title:meta.title,created,publicUrl:meta.visibility==='bearer'&&token?artifactUrl(meta.slug,token):undefined,viewerUrl:meta.visibility!=='bearer'?artifactViewerUrl(meta.slug):undefined,adminUrl:adminUrl(meta.slug),meta:{...meta,tokenHash:undefined}});
  }catch(error){next(error)}});
  app.get('/api/artifacts',async(_req,res,next)=>{try{const metas=await listArtifacts();res.json({artifacts:metas.map((meta)=>({...meta,tokenHash:undefined,viewerUrl:meta.visibility!=='bearer'?artifactViewerUrl(meta.slug):undefined,adminUrl:adminUrl(meta.slug)}))})}catch(error){next(error)}});
  app.get('/api/artifacts/:slug',async(req,res,next)=>{try{const meta=await readMeta(routeParam(req.params.slug));if(!meta)return res.status(404).json({error:'Artifact not found'});return res.json({artifact:{...meta,tokenHash:undefined,viewerUrl:meta.visibility!=='bearer'?artifactViewerUrl(meta.slug):undefined,adminUrl:adminUrl(meta.slug)}})}catch(error){next(error)}});
  ...
  app.get('/v/:slug{/*artifactPath}',async(req,res,next)=>{try{
    ...
    if(!meta||meta.visibility==='bearer'||isExpired(meta))return res.status(404).send('Not found');
    const artifactPath=publicPathFromWildcard(req.params.artifactPath); const current=auth.getRequestViewerSession(req);
    if(!current){
      const original=new URL(req.originalUrl,config.baseUrl); const expectedPrefix=`/v/${encodeURIComponent(slug)}/`;
      if(!original.pathname.startsWith(expectedPrefix))return res.status(400).send('Invalid artifact path');
      const returnPath=`${original.pathname}${original.search}`;
      const handoffId=auth.beginViewerHandoff(slug,returnPath);
      return res.redirect(`${config.appOrigin}/auth/artifacts/authorize/${encodeURIComponent(handoffId)}`);
    }
    if(!canUserViewArtifact(meta,current.user))return res.status(403).send('You do not have access to this artifact');
    artifactHeaders(res,true);
    await sendArtifactPath(req,res,{slug,requestPath:artifactPath,urlPrefix:`/v/${encodeURIComponent(slug)}/${artifactPath}`,allowDirectoryListing:meta.allowDirectoryListing});
  }catch(error){next(error)}});
