...
export const config = {
  ...
  webauthnOrigin,
  webauthnRpId,
  actionLimitPerMinute: Math.max(1, Number(process.env.ACTION_LIMIT_PER_MINUTE ?? 10)),
  actionLimitPerDay: Math.max(1, Number(process.env.ACTION_LIMIT_PER_DAY ?? 100)),
  sessionIdleMs: Math.max(5, Number(process.env.SESSION_IDLE_MINUTES ?? 1_440)) * 60_000,
  recoveryMaxConcurrent: Math.max(1, Number(process.env.RECOVERY_MAX_CONCURRENT ?? 2)),
  hermesRunMaxMs: Math.max(5, Number(process.env.HERMES_RUN_MAX_MINUTES ?? 30)) * 60_000,
...
export function assertProductionSecrets(): void {
  if (process.env.NODE_ENV === 'production') {
    ...
    if (parsedBaseUrl.protocol !== 'https:' || parsedBaseUrl.username || parsedBaseUrl.password || parsedBaseUrl.search || parsedBaseUrl.hash) missing.push('BASE_URL must be a clean HTTPS origin');
    if (parsedAppOrigin.protocol !== 'https:' || parsedAppOrigin.username || parsedAppOrigin.password || parsedAppOrigin.search || parsedAppOrigin.hash) missing.push('APP_ORIGIN must be a clean HTTPS origin');
